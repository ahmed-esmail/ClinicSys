...
exports.addReceptionist = function(request, response, next){
    ...
    else
    {
        ...
        userObject.save()
            .then(object1=>{               
                response.status(201).json({message:"Receptionist added successfully"})
            })
            .catch(error=>{
                error.status=500;
                next(error);
...
exports.updateReceptionist = function(request, response, next){
    ...
    else
    {
        ...
        User.updateOne({_id:request.body.id},
            ...
            .then(result=>{
                response.status(201).json({message:"Receptionist updated successfully"});
            })
            .catch(error=>{
                error.status=500;
                next(error);
...
exports.deleteReceptionist = function(request, response, next){
    ...
    else
    {
        User.deleteOne({_id: request.params.id})
            .then(result=>{
                response.status(201).json({message:"Receptionist deleted successfully"})
            })
            .catch(error=>{
                error.status=500;
                next(error);
