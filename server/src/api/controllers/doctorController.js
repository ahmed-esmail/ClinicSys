...
exports.addDoctor = function(request, response, next){
    ...
    else
    {
        let userObject=new User({
            ...
            profileImg:{ data: fs.readFileSync(path.join(__dirname + './../../../../images/' + request.file.path)),
        ...
        userObject.save()
                .then(object1=>{
                    doctorObject.save()
                        ...
                        .catch(error=>{
                            //delete user
                            User.deleteOne({phoneNumber: userObject.phoneNumber}, function(err) {
                                if(err) throw err;
                            })
                            error.status=500;
                            next(error);
...
exports.addAppointmentToDoctor = function(request, response, next){
    ...
    {
        Doctor.updateOne({_id:request.body.id},
            {
                $push:{
        ...
        Doctor.updateOne({_id:request.body.id},
            ...
            .then(result=>{
                if(result.matchedCount)
                    ...
                    .then(result=> {
                        response.status(201).json({message:"Doctor updated successfully"});
                    })
                    .catch(error=>{
                        error.status=500;
                        next(error);
                ...
                User.deleteOne({_id: request.params.id})
                    .then(result=> {
                        if(result.deletedCount!=0)
                            response.status(201).json({message:"Doctor deleted successfully"})
                        else 
                        {
                            let error = new Error();
                            error.status = 422;
